...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<'advisors' | 'reports' | 'users'>('advisors')
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const [{ data: advisorData }, { data: reportData }] = await Promise.all([
  ...
  useEffect(() => {
    if (tab === 'users' && token && users.length === 0) {
  ...
  const handleDeleteUser = async (userId: string, userEmail: string) => {
    if (!confirm(`「${userEmail}」のアカウントを削除しますか？\n関連するデータ（口コミ等）もすべて削除されます。この操作は取り消せません。`)) return
    ...
    <div>
      ...
      {tab === 'users' && (
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-gray-600">口コミ数</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600">{user.nickname || '未設定'}</td>
                  <td className="px-4 py-3 text-gray-600">{user.review_count}件</td>
          ...
          {users.length === 0 && (
            <div className="text-center py-8 text-gray-400">ユーザーがいません</div>
          )}
        </div>
      )}
